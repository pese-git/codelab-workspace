# Система планирования CodeLab Agent Runtime

**Версия документации:** 1.1
**Дата:** 31 января 2026
**Статус:** 🟢 60% Complete - Ready for Integration

---

## 📚 Документация

Полная архитектурная документация для реализации системы планирования, декомпозиции и исполнения задач.

### Основные документы

| Документ | Описание | Статус |
|----------|---------|--------|
| **[Архитектура системы](planning-system-architecture.md)** | Обзор архитектуры, диаграммы, интеграция | ⭐ Начните отсюда |
| **[Dashboard](PLANNING_SYSTEM_DASHBOARD.md)** | Интерактивный дашборд прогресса | 📊 NEW |
| **[Summary](PLANNING_SYSTEM_SUMMARY.md)** | Executive summary | 📝 NEW |
| **[Final Report](PLANNING_SYSTEM_FINAL_REPORT.md)** | Детальный отчёт реализации | 📋 NEW |

### Дизайн компонентов

| Документ | Описание | Статус |
|----------|---------|--------|
| **[FSM Orchestrator](fsm-orchestrator-design.md)** | Конечный автомат для управления состоянием | ✅ Реализовано |
| **[Task Classifier](task-classifier-design.md)** | Классификация задач | ✅ Реализовано |
| **[Execution Engine](execution-engine-design.md)** | Движок исполнения планов | ✅ Реализовано |
| **[Plan Repository](plan-repository-design.md)** | Персистентность планов в БД | ✅ Реализовано |
| **[Execution Architecture](execution-engine-architecture.md)** | Архитектура execution layer | 🎨 NEW |

### Руководства разработчика

| Документ | Описание | Аудитория |
|----------|---------|-----------|
| **[Quick Start](../codelab-ai-service/agent-runtime/doc/PLANNING_SYSTEM_QUICKSTART.md)** | Быстрый старт | Разработчики |
| **[Execution Engine Guide](../codelab-ai-service/agent-runtime/doc/EXECUTION_ENGINE_GUIDE.md)** | Руководство по ExecutionEngine | Разработчики (NEW) |
| **[Execution Layer README](../codelab-ai-service/agent-runtime/doc/EXECUTION_LAYER_README.md)** | Quick reference | Разработчики (NEW) |

---

## 🎯 Цель системы

Реализовать **детерминированный механизм планирования** для мультиагентной системы CodeLab, обеспечивающий:

✅ Корректное разделение сложных задач на подзадачи  
✅ Распределение подзадач между специализированными агентами  
✅ Контроль исполнения и обработку ошибок  
✅ Исключение самопереключений и циклов между агентами  
✅ Воспроизводимое и управляемое поведение LLM

---

## 🏗️ Архитектура на высоком уровне

```
┌─────────────────────────────────────────────┐
│           SessionsRouter (API)              │
└────────────────────┬────────────────────────┘
                     │
┌────────────────────▼────────────────────────┐
│    MessageOrchestrationService (Facade)     │
├────────────────────────────────────────────┤
│  ┌──────────────┐  ┌──────────────┐       │
│  │ FSMOrchest.  │  │TaskClassif.  │       │
│  └──────────────┘  └──────────────┘       │
│  ┌──────────────┐  ┌──────────────┐       │
│  │ExecEngine    │  │PlanRepo      │       │
│  └──────────────┘  └──────────────┘       │
└────────────────────────────────────────────┘
                     │
┌────────────────────▼────────────────────────┐
│          Agent Layer (5 agents)             │
│  Orchestrator, Coder, Architect,            │
│  Debug, Ask                                 │
└────────────────────────────────────────────┘
```

---

## 📊 Основные компоненты

### 1. FSM Orchestrator
**Управление жизненным циклом задачи через конечный автомат**

Состояния:
- `IDLE` → Ожидание задачи
- `CLASSIFY` → Классификация (атомарная vs неатомарная)
- `PLAN_REQUIRED` → Требуется план
- `ARCHITECT_PLANNING` → Architect создает план
- `EXECUTION` → Исполнение плана
- `ERROR_HANDLING` → Обработка ошибки
- `COMPLETED` → Завершение

📖 [Детальный дизайн](fsm-orchestrator-design.md)

### 2. Task Classifier
**Определение типа задачи и целевого агента**

- Использует LLM для классификации
- Fallback на keyword matching
- **Ключевое правило:** `is_atomic=false` → `agent=plan`

📖 [Детальный дизайн](task-classifier-design.md)

### 3. Execution Engine ✅
**Исполнение плана с управлением зависимостями**

- ✅ Разрешение зависимостей между subtasks
- ✅ Параллельное выполнение независимых задач
- ✅ Батчирование с учётом max_parallel_tasks
- ✅ Обработка ошибок и cancellation
- ✅ Мониторинг прогресса и агрегация результатов

📖 [Детальный дизайн](execution-engine-design.md) | 🎨 [Архитектура](execution-engine-architecture.md) | 📚 [Руководство](../codelab-ai-service/agent-runtime/doc/EXECUTION_ENGINE_GUIDE.md)

### 3.1. SubtaskExecutor ✅
**Выполнение одной подзадачи в целевом агенте**

- ✅ Маршрутизация к агентам (Coder, Debug, Ask)
- ✅ Контекст с результатами зависимостей
- ✅ Retry logic для failed subtasks
- ✅ Обновление статусов в БД

📖 [Код](../codelab-ai-service/agent-runtime/app/domain/services/subtask_executor.py) | 🧪 [Тесты](../codelab-ai-service/agent-runtime/tests/test_subtask_executor.py)

### 4. Plan Repository
**Персистентность планов в БД**

- SQLAlchemy модели
- Миграция Alembic
- Маппер Domain ↔ DB

📖 [Детальный дизайн](plan-repository-design.md)

---

## 🔑 Ключевые инварианты

### Инвариант 1: Single Source of Truth
- **Только Orchestrator** может переключать агентов
- **Только ExecutionEngine** управляет исполнением

### Инвариант 2: Agent Isolation
- Агенты **не могут** вызывать `switch_mode`
- Все переключения **только** через FSM

### Инвариант 3: Plan Atomicity
- Каждый subtask **атомарный**
- Нет self-assignment для Architect

### Инвариант 4: Determinism
- Одинаковый input → одинаковый output
- Система воспроизводима для отладки

---

## 📋 Критерии готовности (Definition of Done)

- [ ] ✅ Сложная задача не уходит напрямую в Coder
- [ ] ✅ Architect не может исполнять код
- [ ] ✅ Нет self-switching агентов
- [ ] ✅ Все задачи завершаются через FSM
- [ ] ✅ Поведение системы воспроизводимо

---

## 🚀 План разработки

**Фаза 1 (Неделя 1):** Проектирование и подготовка  
**Фаза 2 (Неделя 2-3):** Базовая инфраструктура (Classifier, Repository)  
**Фаза 3 (Неделя 4-5):** Execution Engine  
**Фаза 4 (Неделя 6):** Интеграция с Orchestrator  
**Фаза 5 (Неделя 7):** Runtime Guards и валидация  
**Фаза 6 (Неделя 8):** Тестирование и документация

📖 Подробный план: см. [План разработки](../DEVELOPMENT_PLAN.md)

---

## 🔧 Технический стек

- **Python 3.10+**
- **FastAPI** - REST API
- **SQLAlchemy** - ORM
- **Alembic** - миграции БД
- **Pydantic** - валидация
- **pytest** - тестирование
- **PostgreSQL/SQLite** - хранилище

---

## 📖 Как читать документацию

### Для разработчиков, начинающих с нуля:

1. Прочитайте [Архитектуру системы](planning-system-architecture.md) - понимание общей картины
2. Прочитайте дизайн компонента, который вы реализуете:
   - Task Classifier → [task-classifier-design.md](task-classifier-design.md)
   - FSM Orchestrator → [fsm-orchestrator-design.md](fsm-orchestrator-design.md)
   - Execution Engine → [execution-engine-design.md](execution-engine-design.md)
   - Plan Repository → [plan-repository-design.md](plan-repository-design.md)
3. Следуйте структуре файлов и сигнатурам методов
4. Пишите тесты

### Для code review:

1. Проверьте против инвариантов системы
2. Проверьте соответствие дизайну
3. Проверьте тесты (>85% coverage)
4. Проверьте логирование

---

## 🎓 Важные концепции

### Атомарная задача
Выполняется одним агентом за один шаг:
- Нет анализа проекта
- Нет архитектурных решений
- Нет создания системы
- Пример: "Create a button component"

### Неатомарная задача
Требует планирования:
- Требует анализа проекта
- Требует архитектурных решений
- Требует создания системы
- Пример: "Design and implement authentication system"

### Правило классификации
```
if is_atomic == false:
    agent MUST BE "plan"  # Обязательно!
```

---

## 🧪 Тестирование

### Unit тесты
```bash
# Все тесты системы планирования
cd codelab-ai-service/agent-runtime
uv run pytest tests/test_task_classifier.py tests/test_fsm_orchestrator.py tests/test_subtask_executor.py tests/test_execution_engine.py -v

# Отдельные компоненты
uv run pytest tests/test_fsm_orchestrator.py -v
uv run pytest tests/test_task_classifier.py -v
uv run pytest tests/test_subtask_executor.py -v  # NEW
uv run pytest tests/test_execution_engine.py -v  # NEW
```

### Integration тесты
```bash
pytest tests/e2e/test_planning_workflow.py -v
```

### Coverage
```bash
pytest --cov=app tests/ --cov-report=html
```

---

## 📈 Метрики успеха

- **Test Coverage:** > 85%
- **E2E Tests Pass Rate:** 100%
- **Performance:** plan execution < 5s overhead
- **Classification Accuracy:** > 90%
- **Plan Success Rate:** > 85%

---

## 🔗 Связанные документы

- [Аудит текущей реализации](../doc/audit-report.md)
- [План разработки](../DEVELOPMENT_PLAN.md)
- [Архитектура Agent Runtime](../agent-runtime/README.md)

---

## 💡 Часто задаваемые вопросы

**Q: Почему Architect не может быть назначен на subtask?**  
A: Architect - это планировщик, а не исполнитель. Он анализирует и создает планы. Исполнение - это работа для Coder, Debug и Ask.

**Q: Почему у всех неатомарных задач agent = "plan"?**  
A: Потому что неатомарные задачи требуют декомпозиции. Только Architect может разбить сложную задачу на subtasks.

**Q: Что делать если subtask падает?**  
A: ExecutionEngine автоматически маршрутизирует в Debug агент для анализа ошибки.

**Q: Можно ли запустить subtasks параллельно?**  
A: Да, если они независимы (нет зависимостей между ними).

---

## 📞 Контакты

- **Architecture:** Sergey Penkovsky
- **Questions:** Создавайте issues в GitHub

---

**Last updated:** 31 января 2026
**Статус:** ✅ 60% Complete - Execution Layer Ready
